...
for qid in tqdm(run):
    ...
    # 1. single-list sampling + coverage-based filtering (including the scores)
    ...
    if len(filterd_top20) > 0:
        dataset_dict['pos_20.neg_51'].append({
            ...
            'positive_document_scores': [run[qid][docid] for docid in filterd_top20],
            'negative_document_scores': [run[qid][docid] for docid in document_ids_all[50:]],
            'answer': None,
    ...
    for positive_category, negative_category in [
        ...
        if len(positive_docs) > 0:
            dataset_dict[tag].append({
                ...
                'positive_document_scores': [run[qid][docid] for docid in positive_docs],
                'negative_document_scores': [run[qid][docid] for docid in negative_docs],
                'answer': None,
                'source': f'crux-researchy.tau:{tau}.clueweb22-B',
            })
