...
# new_dataset_name = 'msmarco-passage-new-pretokenized'
...
for split in ['train', 'eval']:
    ...
    if split == 'eval':
        pt_dataset = QrelDataset(data_args)

    # Convert to hf
    hf_data = []
    for i in tqdm(range(len(pt_dataset)), desc="Converting to HF dataset"):
        query, passages = pt_dataset[i]
        hf_data.append({"query": query[0], "passage_group": [p[0] for p in passages] })
    ...
    # Apply tokenization
    tokenized_hf_dataset = hf_dataset.map(pretokenize, num_proc=64)
...
tokenized_hf_dataset.save_to_disk(f"/scratch-shared/dju/datasets/{new_dataset_name}")
